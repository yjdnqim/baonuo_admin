...
<#list fieldList as var>
	...
	*/
	...
	public ModelAndView goView(){
		...
		PageData pd = this.getPageData();
		try {
			PageData ${objectNameLower}Pd = ${objectNameLower}Service.findById(pd);	//根据ID读取
			mv.setViewName("${packageName}/${objectNameLower}/${objectNameLower}_view");
			mv.addObject("pd", pd);
			mv.addObject("${objectNameLower}", ${objectNameLower}Pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}
		return mv;
